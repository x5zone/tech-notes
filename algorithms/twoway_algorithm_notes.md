...
- **问题背景**
  ...
  - 在预处理阶段（即 `maxSuf` 函数），算法并不是一上来就在“寻找周期”，而是通过字典序比较，逐步排除一整段**必然失败的位移范围**。每一次比较的结果，都会让一部分模式串右移尝试被永久否定。算法中维护的一个核心位移变量（即 `p`），其所记录的正是当前已知信息下仍有可能成功的最小位移界限：所有小于该跨度的右移，都已经被证明必然失败，无需再尝试。
  - 从程序员的视角看，字符串搜索算法中最直观的操作，就是不断决定"模式串能否继续右移、以及最少要右移多少"。为了贴合这种直觉，本文将把这类已被证明可以直接跳过的位移范围，非正式地称为一种安全跳跃距离（Safe Shift Distance）。
  - 至于“周期”的数学概念，本文不会作为预设前提强行引入，而是让它在算法推进的过程中，作为这些跳跃决策在局部范围内自然显现的结构性质逐步浮现。这样，工程上的决策逻辑与数学结构之间的联系，才能以更易理解的方式呈现。

- **本文定位**
  - 本文并不试图提供严谨的数学证明，而是尝试从程序员视角出发，通过字典序约束和几何位移分析，把 Two-Way 还原为一系列可执行的工程决策：
    - 哪些位移已经被排除。
    - 哪些位置仍值得一试。
    - 为什么整套流程在 $O(1)$ 额外空间下依然可以保证 $O(n)$ 的时间复杂度。
  - 作为 Rust 初学者，我在阅读《[Inside Rust Std Library](https://github.com/Warrenren/inside-rust-std-library)》一书时接触到了 Two-Way 算法。通过网络搜索和 AI 讲解资料参考，往往泛泛而谈或过于数学化，难以透彻理解代码，遂记录下个人的推导与理解过程。

- **算法价值**
  - Two-Way 算法已成为字符串匹配领域的重要工程实现之一——不仅 Rust 标准库，glibc 和 Go 的内部实现亦采用了它，极具学习与研究价值。
  - 与 KMP 的前缀函数和 BM 的多表启发式相比，Two-Way 在常数空间内完成确定性的线性时间匹配，同时避免频繁的状态回退和复杂预处理，缓存更友好、运行更稳定，更适合作为标准库级别的通用实现。
...
- **重要说明**
  ...
  - **“最长串获胜”原则（或“短串必败”原则）**：
    - **核心逻辑**：在字典序比较中，如果短串 $A$ 恰好是长串 $B$ 的前缀（即 $B = A + \text{suffix}$），且 $B$ 为最大后缀，那么因空字符最小，较长的串 $B$ 必然大于短串 $A$。
...
- **曲线可视化**
  ...
  ```
    ...
    /    \  /
          \/
  ```
  - 字符字典序的变化可以视作这条曲线的起伏，整个字符串中存在多个波峰和波谷。

- **波峰与波谷的对应关系**
  - 最大波峰对应字典序升序（`≤`方向）的临界分解点（`maxSuf`），最大波谷对应字典序降序（`≥`方向）的临界分解点（`maxSufTilde`）。 

- **“刺头”比喻**
  - 这些关键点就像模式字符串中的“刺头”——当它们与搜索串中已匹配的“镜像串”对齐时，一旦右移跳跃，就很容易导致不匹配。抓住它们的规律，就能在匹配过程中为排除显然无效的位移提供依据，从而更有针对性地选择跳跃步数。
...
- **应用示例**
  - 例如：模式串 `S` 的最大波峰位于位置 `i`，最大波谷位于位置 `j`。假设我们已经匹配了若干字符，构成了镜像串。在一次搜索过程中，如果右移步数不恰当，模式串中的“刺头”就会对准镜像串中的字符。“刺头”可没那么容易匹配上，这通常会是一轮注定失败的尝试。所以，更明智的选择是，根据“刺头”和镜像串之间的关系，排除一批显然不划算的位移，再在剩余的候选中去思考“安全跳跃距离”——所有的“魔法”，都隐藏在刺头字符与镜像串的互动之中。
...
- **博弈规则**
  - **挑战失败（a < b）**：挑战者在某个深度露了怯（字典序更小），不仅自己被淘汰，由于与擂主前 $k$ 个字符相同，这 $k$ 个位置也被拖累而排除。
  - **势均力敌（a == b）**：双方暂时打平，继续向后比对。
  - **挑战成功（a > b）**：挑战者展现出了更强的实力（字典序更大），直接将旧擂主踢下台，自己成为新擂主，并重置赛场。
...
- **理解难点**
  - 需要在一开始就明确指出的是：maxSuf 并不是一个语义自洽、可以脱离上下文独立理解的函数。
...
- **正确视角**
  - 这些易带来困扰的问题，是因为 maxSuf 的设计目标，本就不在于"单独成立"。
...
- **三种情况对照表**
  - | 情况 | 代码 | 直观理解 |
    |-----|------|---------|
    | **a < b** | `j += k; k = 1; *p = j - ms;` | **挑战失败**：挑战者太弱，不仅自己输了，还拖累了后续 $k$ 个位置，在当前对战条件下被一并淘汰。 |
    | **a == b** | `k++` 或 `j += *p` | **势均力敌**：双方打平。要么继续深入（`k++`），要么利用已知的重复结构跳过必败区间（`j += *p`）。 |
...
#### 3.5.1 为什么 `j += k`？
  ...
  - **一句话总结**
    - 以 `j + 1` 开始的 k 个字符，全部必输，没有挑战的必要了，可以直接跳过
  ...
  - **历史回溯视角（同一论证的不同视角）**
    ...
    - 假设尝试从 j+d（1 ≤ d < k）开始挑战，由于 x[ms+d...] 作为长串都没能战胜擂主，作为其"短版本"的 x[j+d...] 在同一区间内也绝无获胜可能
    - 因此与其重复历史上已经失败过的路径，不如直接跳过这 k 个无效起点

  - **为什么是 j += k？**
    - 既然这 k 个位置都没有夺冠可能，直接执行 `j += k`
    - **陷阱提示**：执行 `j += k` 后，真正的下一个挑战起点是 x[j+k+1] 而非 x[j+k]，这是一个容易忽略的"+1"偏移陷阱
...
#### 3.5.4 几何对齐与安全跳跃证明
  ...
  - **为什么 `*p = j - ms` 是最小安全跳跃距离？**
    ...
    - **深入分析——证明第二类 d 根本不存在**
      ...
      - **回顾 `maxSuf` 执行历史**
        - 在算法运行过程中，区间 $(ms, j)$ 内的每一个位置都曾为挑战者 $j$ 。
      - **跳跃后的位置关系**
      ...
      - **起点落在闭区间 $[ms+2, j_{old}]$**
        ...
        - 既然挑战者 j 已演进到现在的 $j_{old}$（起始字符为 $j_{old}+1$），说明这些位置并非因"够不到结尾"而失败，而是在模式串内部就撞到了比自己大的字符。因为挑战者 j 存在（说明它们必定失败且只会比挑战者 j 更早失败），失败原因必是实打实的 a < b 导致的提前失败。因此，镜像区的匹配长度足以覆盖这些失配位置，从而确保失配。
...
- **k == p：发现重复副本**
  - 比较点 `ms+p` 恰好等于 `j`，说明挑战者是擂主的一个重复副本
  - **最长串获胜**：擂主作为更长的串，字典序上必胜，挑战者无需继续比较
  - 直接跳到 `j + p`。因为任何位移 d < p 的起点，在擂主赛历史中都曾是挑战者，且都已失败告终——这些位置作为挑战者时，要么因 a < b 落败，要么因打平后被更长串淘汰。它们是"必败起点"，无需再次尝试。
